...
int DiffMode = 1;
	if (fTestNet) {
		...
		if (pindexLast->nHeight+1 >= WTMINT_KGW_StartBlock) { DiffMode = 2; }
	}
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            if (!MoneyRange(coins.vout[prevout.n].nValue) || !MoneyRange(nValueIn)){
				//printf("coins.vout[prevout.n].nValue=%llu, nValueIn=%llu",coins.vout[prevout.n].nValue,nValueIn);
...
bool CBlock::ConnectBlock(CValidationState &state, CBlockIndex* pindex, CCoinsViewCache &view, bool fJustCheck)
{
	...
	int64 blockValue = WTMint::GetBlockValue(diffTime,prevBits, pindex->nHeight, nFees, prevHash);
...
int GetAuxPowStartBlock()
{
   if (fTestNet)
        ...
        return WTMINT_AUX_StartBlock;//170500; 
}
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
		const char* pszTimestamp = WTMINT_pszTimestamp;//"Temple coin was born on March 25th 2014 by TempleCoin.com";
		...
		txNew.vout[0].scriptPubKey = CScript() << ParseHex(WTMINT_scriptPubKey) << OP_CHECKSIG;
	...
	//tcgen
        block.nTime    = WTMINT_BLOCK_nTime;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = WTMINT_BLOCK_nNonce;
...
////////////////////////////////////////////////////////
	 ...
	 assert(block.hashMerkleRoot == uint256(WTMINT_BLOCK_hashMerkleRoot));
...
CBlockTemplate* CreateNewBlock(const CScript& scriptPubKeyIn)
{
    ...
    int64 nFees = 0;
    {
        ...
        pblock->vtx[0].vout[0].nValue = WTMint::GetBlockValue(diffTime,pindexPrev->nBits, pindexPrev->nHeight+1, nFees, pindexPrev->GetBlockHash());
